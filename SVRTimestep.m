...
% ใช้ข้อมูลย้อนหลัง 7 วัน
lookback = 30;
num_features = length(feature_cols);
num_samples = size(X_raw, 1) - lookback;

X = zeros(num_samples, lookback * num_features);
y = zeros(num_samples, 1);

for i = 1:num_samples
    window = X_raw(i:i+lookback-1, :);  % 7 วันย้อนหลัง
    X(i, :) = reshape(window', 1, []);  % flatten (1×70)
    y(i) = y_raw(i + lookback);         % ฝนของวันถัดจาก window
end
...
save('modelGUISVR_Lookback30.mat', 'mdl_svr', 'ps', 'lookback', 'feature_cols');
...
sgtitle('SVR: Predict Next-Day Rainfall using Previous 7 Days');
